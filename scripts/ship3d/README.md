...
| `fauna3d.py` | blender | **roaming-fauna walk-sheets** → `assets/sprites/fauna/<species>.png` + `fauna_manifest.ron`. 3D toon animals in the RPG-Maker layout (3 cols still/w1/w2 × 4 rows down/left/right/up) so the game reuses `CharacterAnim`/`Facing`. Each animal is modelled facing −Y and spun about Z for the four facings; walk frames shuffle diagonal leg pairs. Consumed by `src/surface_fauna.rs` (roamers), distinct from the static peekers in `objects3d.py`. |
